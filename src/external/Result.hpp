...
namespace gdml {
    ...
    struct Result {
        ...
        template<typename = std::enable_if_t<!std::is_same_v<T, E>>>
        Result(T const& value) : Result(types::Ok<T>(value)) {}
        
        template<typename = std::enable_if_t<!std::is_same_v<T, E>>>
        Result(T&& value) : Result(types::Ok<T>(std::move(value))) {}

        template<typename = std::enable_if_t<!std::is_same_v<T, E>>>
        Result(E const& value) : Result(types::Err<E>(value)) {}

        template<typename = std::enable_if_t<!std::is_same_v<T, E>>>
        Result(E&& value) : Result(types::Err<E>(std::move(value))) {}
    ...
    struct Result<void, E> {
        ...
        typedef details::Storage<void, E> storage_type;
        ...
        Result(Result&& other) {
            if (other.isOk()) {
                details::Constructor<void, E>::move(std::move(other.storage_), storage_, details::ok_tag());
                ok_ = true;
            } else {
                details::Constructor<void, E>::move(std::move(other.storage_), storage_, details::err_tag());
                ok_ = false;
            }
        }
        ...
        Result(const Result& other) {
            if (other.isOk()) {
                details::Constructor<void, E>::copy(other.storage_, storage_, details::ok_tag());
                ok_ = true;
            } else {
                details::Constructor<void, E>::copy(other.storage_, storage_, details::err_tag());
                ok_ = false;
            }
        }
        ...
        void expect(const char* str) const {
            if (!isOk()) {
                std::fprintf(stderr, "%s\n", str);
                std::terminate(); 
            }
        ...
        template<typename Func>
        Result<void, E> then(Func func) const {
            return details::then(*this, func);
        }

        template<typename Func>
        Result<void, E> otherwise(Func func) const {
            return details::otherwise(*this, func);
        }
